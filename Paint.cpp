...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	HWND hWndToolbar = CreateSimpleToolbar(hWnd);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		case ID_FILE_OPEN:
		{
			...
			ofn.lpstrFilter = _T("All Files *.*\0*.*\0Text Files *.txt\0*.TXT\0*.bmp\0*.bmp\0");
			ofn.nFilterIndex = 1;
		...
		case ID_FILE_SAVEAS:
		{
			...
			ofn.nFilterIndex = 1;
			ofn.lpstrInitialDir = _T("C:\\");
			ofn.lpstrTitle = _T("My Application - Save As file");
			ofn.lpstrDefExt = _T("bmp");
			ofn.Flags = OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST;
			// Display the Open dialog box. 
		...
		case ID_FILE_NEW:
		{
			MessageBox(NULL, _T("NOT FIND!"), _T(""), MB_OK);
			break;
		}
		case ID_FILE_SAVE:
		{
			if (check == true)
			{
				...
				PBITMAPINFO PBI = CreateBitmapInfoStruct(hWnd, hBitmap);
				CreateBMPFile(hWnd, save, PBI, hBitmap, hdc);
				break;
			}
		}
		break;
...
VOID ChangePenColor(HDC hdc)
{
	...
	case 0:
		...
		break;
	case 1:
		hBrush = CreateHatchBrush(HS_CROSS, shapeColor);
		break;
	case 2:
...
INT_PTR CALLBACK Text(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	HWND xlocaHandle = GetDlgItem(hDlg, IDC_XLOCATION);
	HWND ylocaHandle = GetDlgItem(hDlg, IDC_YLOCATION);
	HDC texthdc = GetDC(textHandle);
	HDC mainhdc = GetDC(hWnd);
	HDC subhdc = GetDC(hDlg);
	HDC comphdc = CreateCompatibleDC(subhdc);
	RECT textbox;
	...
	case WM_COMMAND:
		...
		case IDC_FONT:
			if (ChooseFont(&cf) == TRUE)
			{
				...
				SendMessage(textHandle, WM_SETFONT, (WPARAM)hfont, 0);
				
				//SelectObject(texthdc, hfont);
				//SelectObject(comphdc, hfont);
				//SetTextColor(mainhdc, rgbCurrent);
				//SetTextColor(comphdc, rgbCurrent);
		...
		case IDOK:
			wchar_t *pEnd;
			GetWindowText(xlocaHandle, buffer, 1024);
			x = wcstol(buffer, &pEnd, 10);
			GetWindowText(xlocaHandle, buffer, 1024);
			y = wcstol(buffer, &pEnd, 10);
			...
			SetTextColor(mainhdc, rgbCurrent);
			SetTextColor(comphdc, rgbCurrent);
			TextOut(mainhdc, x, y, buffer, len);
			TextOut(comphdc, x, y, buffer, len);
			DeleteDC(comphdc);
			ReleaseDC(hDlg, subhdc);

			SelectObject(mainhdc, hfontPrev);
			SelectObject(comphdc, hfontPrev);
			SetTextColor(mainhdc, rgbPrev);
			SetTextColor(comphdc, rgbPrev);
...
HWND CreateSimpleToolbar(HWND hWndParent)
{
	...
	HWND hWndToolbar = CreateWindowEx(0, TOOLBARCLASSNAME, NULL,
		WS_VISIBLE | WS_CHILD | WS_BORDER | TBSTYLE_FLAT | TBSTYLE_WRAPABLE, 0, 0, 0, 0,
	...
	TBBUTTON tbButtons[numButtons] =
	{
		...
		{ MAKELONG(STD_FILESAVE, ImageListID),ID_FILE_SAVEAS,TBSTATE_ENABLED, buttonStyles,{ 0 }, 0, (INT_PTR)L"Save As" },
		{ MAKELONG(STD_CUT,      ImageListID), ID_FILE_SAVE,  0, 0,{ 0 }, 0, (INT_PTR)L"Cut" },
		{ MAKELONG(STD_COPY,     ImageListID), ID_FILE_SAVE,  0, 0,{ 0 }, 0, (INT_PTR)L"Copy" },
		{ MAKELONG(STD_PASTE,    ImageListID), ID_FILE_SAVE,0, 0,{ 0 }, 0, (INT_PTR)L"Paste" }
